...
export interface ISVInitSaleOptions {
...
export interface ISVRefundTransactionOptions {
  /** Transaction session identification */
  sessionId: string;
  /** The ID of the target terminal. This can be found either by using the Search POS Devices API call, or within the Viva Terminal app (More > About > "Source Terminal ID") */
  terminalId: string;
...
export interface ISVCreateActionOptions {
  ...
  request: {
    /** Contains the control token with the AADE token, passing the session key and KCV as per AADE specifications */
    token: string;
  };
}

export interface ISVCreateActionReturn {
...
export interface ISVGetActionDatasOptions {
  /** Action ID by which the data should be filtered */
  actionId: string;
}

export interface ISVGetActionDatasReturn {
  /** Terminal identification */
  terminalId: string;
  /** Cash register identification */
  cashRegisterId: string;
